/* LUX//CLEARANCE live feed loader — data comes from the verified background scanner */
(()=>{
...
async function loadFeed(manual=false){
 ...
 status(manual?'Refreshing live catalog…':'Loading live catalog…','Fetching the latest verified Walmart clearance feed.');
 try{
  ...
  saveState();render();
  status(items.length?'LIVE CATALOG':'Catalog warming up',items.length?`${items.length.toLocaleString()} in-stock clearance items sold by Walmart · updated ${ageLabel(data.updated_at)}`:'The live scanner has not published its first verified catalog yet.');
 }catch(e){status('Catalog unavailable',`Could not load the live feed: ${e.message}`)}finally{loading=false}
}
